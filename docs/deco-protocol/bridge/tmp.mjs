...
// is self-checked (tmp.test.mjs); the on-wire session is code-derived only.
//
...
export function appV2Request(opcode, body, txid) {
...
export function parseAppV2(payload) {
  return {
...
export class DecoTmp {
  ...
  #serial = 1
  #txid = 1
  #assoc = null // resolves once version-associated
  #assocPromise = null
  #pending = new Map() // serial -> {resolve, reject, frags}
  #onData
  ...
  #onDataFrame(f) {
    ...
    try {
      ...
      if (code != null && code !== 0) {
        const e = new Error(`Deco opcode error ${code}${json.msg ? `: ${json.msg}` : ""}`)
        e.code = code
        p.reject(e)
      } else {
  ...
  associate() {
    if (this.#assoc) return this.#assocPromise
    this.#assocPromise = new Promise((resolve, reject) => {
    ...
    }).then(() => this.#sock.write(tmpShort(TMP_TYPE.RSP))) // ACK
...
// name -> {read, write?} opcodes. b64 fields (ssid/password/name/service_name) are the caller's
// job to encode, same as the HTTP client.
export const OPS = {
